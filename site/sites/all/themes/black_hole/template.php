...
$roundcorners = theme_get_setting('roundcorners');
  ...
  $vars['page_b'] = ($vars['is_front']) ? '<div class="by"><a href="http://www.draculea.net">Draculea</a></div>' : '<div class="by"><a href="http://www.draculea.net">Dracula</a></div>';
}
...
function black_hole_breadcrumb($variables) {
 ...
 */
function black_hole_preprocess_search_result(&$vars) {
 ...
 */
function black_hole_field__taxonomy_term_reference($variables) {
  $output = '';
 ...
 */
function black_hole_file($element) {
...
$preload = theme_get_setting('cssPreload'); // print the js file if css image preload enabled
  if ($preload == '1'){
    drupal_add_js(drupal_get_path('theme','black_hole').'/js/preloadCssImages.jQuery_v5.js');
